...
A PR description states three things and nothing else: what changed, why, and how it was verified.
Keep changes scoped to the intended feature or fix, and rebase on the target branch rather than
...
All commits in this repository are authored solely by `Yihong Lu <willu.star@gmail.com>`.

Do not add `Co-authored-by:` trailers of any kind, and do not add generation footers or
tooling attribution to commit messages or PR descriptions. See `CLAUDE.md` section 1 for the
full policy, which takes precedence over anything in this file.
